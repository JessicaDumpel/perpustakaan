<?php
include "inc/Connection.php";
include "app/Peminjaman.php";
include "app/Anggota.php";
include "app/Buku.php";
$buku = new Buku();
$anggota = new Anggota();
$dtBuku = $buku->tampil();
$dtAnggota = $anggota->tampil();

$pjm = new Peminjaman();
$item = $pjm->get_id($_GET['id_peminjaman']);
?>
<div class="col-md-12">
    <div class="card">
        <div class="card header">
            Ubah Peminjaman
        </div>
        <div class="card-body">
            <form action="/perpustakaan/app/proses.php" method="post">
                <div class="form-group">
                    <label for="">TANGGAL_PINJAM</label>
                    <input type="hidden" class="form-control" name="id_peminjaman" value="<?= $item['id_peminjaman'] ?>">
                    <input type="date" class="form-control" name="tgl_pinjam" value="<?= $item['tgl_pinjam'] ?>">
                </div>
                <div class="form-group">
                    <label for="">TANGGAL_KEMBALI</label>
                    <input type="date" class="form-control" name="tgl_kembali" value="<?= $item['tgl_kembali'] ?>">
                </div>
                <div class="form-group">
                    <label for="">PEMINJAM</label>
                    <select class="form-control" name="id_anggota">
                        <option>Pilih Anggota</option>
                        <?php foreach ($dtAnggota as $key => $item) : ?>
                            <option value="<?= $item['id_anggota'] ?>"><?= $item['nama'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="">BUKU</label>
                    <select class="form-control" name="id_buku">
                        <option>Pilih Buku</option>
                        <?php foreach ($dtBuku as $key => $item) : ?>
                            <option value="<?= $item['id_buku'] ?>"><?= $item['judul'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <button type="submit" name="ubah_peminjaman" class="btn btn-primary btn-sm">Simpan</button>
                </div>
            </form>
        </div>
    </div>
</div>